...
fn main() {
    ...
    // The `cpp` feature's cxx C++ glue is generated by the consumer's Corrosion
    // target (corrosion_add_cxxbridge), not here — compiling it in build.rs too
    // would duplicate the cxxbridge1$* symbols. The `cpp` feature only selects
    // the `cxx` runtime dependency.
